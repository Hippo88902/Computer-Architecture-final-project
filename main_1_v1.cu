...
__global__ void create_ifft_table(complex fft_table[], int window_size) {
	int i = 0, TotalThread = gridDim.x*blockDim.x;
	int strip = window_size / TotalThread;
	int head = (blockIdx.x*blockDim.x + threadIdx.x)*strip;
...
void fft_reorder(complex *fft_window_1, complex *fft_window_2, int window_size, int stage, bool offset) {
	int i, j, reverse;
	for (i = 0; i < window_size; i++) {
		...
		(fft_window_2 + reverse)->real = (offset) ? (fft_window_1 + i)->real / (double)window_size : (fft_window_1 + i)->real;
		(fft_window_2 + reverse)->imaginary = (offset) ? (fft_window_1 + i)->imaginary / (double)window_size : (fft_window_1 + i)->imaginary;
	}
}
...
int main()
{
	...
	FILE *time_list = fopen("time.txt", "wb");
	...
	cudaMemcpy(fft_table, d_fft_table, sizeof(complex)*window_size, cudaMemcpyDeviceToHost);
	...
	for (s = 0; s < second; s++) {
		for (i = 0; i < window_size; i++) {
			...
			fprintf(frequency_list, "%lf\n", (freq_signal + s * window_size + i)->real);
		}
		...
		fft_reorder(fft_output, freq_signal + s * window_size, window_size, stage, false);
		
		for (i = 0; i < window_size; i++) {
			fprintf(frequency_list, "%lf %lf j\n", (freq_signal + s * window_size + i)->real, (freq_signal + s * window_size + i)->imaginary);
		}
	}
	cudaFree(d_fft_table); cudaFree(d_fft_input_1); cudaFree(d_fft_input_2); cudaFree(d_fft_output);
	free(fft_table); free(fft_input_1); free(fft_input_2); free(fft_output); free(time_signal);

	//IFFT
	
	complex *ifft_table = (complex *)calloc(window_size, sizeof(complex));
	complex *d_ifft_table = 0; cudaMalloc((void **)&d_ifft_table, sizeof(complex)*window_size);
	create_ifft_table << < dimGrid, dimBlock >> > (d_ifft_table, window_size);
	cudaMemcpy(ifft_table, d_ifft_table, sizeof(complex)*window_size, cudaMemcpyDeviceToHost);

	complex *restore_signal = (complex *)calloc(window_size*second, sizeof(complex));
	complex *ifft_output = (complex *)calloc(window_size, sizeof(complex));
	complex *ifft_input_1 = (complex *)calloc(window_size, sizeof(complex));
	complex *ifft_input_2 = (complex *)calloc(window_size, sizeof(complex));

	complex *d_ifft_input_1 = 0; cudaMalloc((void **)&d_ifft_input_1, sizeof(complex)*window_size);
	complex *d_ifft_input_2 = 0; cudaMalloc((void **)&d_ifft_input_2, sizeof(complex)*window_size);
	complex *d_ifft_output = 0; cudaMalloc((void **)&d_ifft_output, sizeof(complex)*window_size);

	for (s = 0; s < second; s++) {
		for (i = 0; i < window_size; i++) {
			*(ifft_output + i) = *(freq_signal + s * window_size + i);
		}
		for (i = 0; i < stage; i++) {
			k = (int)pow(2, i);
			points = (int)pow(2, stage - i);
			for (j = 0; j < window_size; j++) {
				if ((j % points) < points / 2) {
					*(ifft_input_1 + j) = *(ifft_output + j);
					*(ifft_input_2 + j) = *(ifft_output + j + (int)points / 2);
					(scale + j)->real = 1.0; (scale + j)->imaginary = 0.0;
				}
				else {
					(ifft_input_1 + j)->real = (ifft_output + j)->real * -1.0;
					(ifft_input_1 + j)->imaginary = (ifft_output + j)->imaginary * -1.0;
					*(ifft_input_2 + j) = *(ifft_output + j - (int)points / 2);
					*(scale + j) = *(ifft_table + ((j%points) - points / 2)*k);
				}
			}
			cudaMemcpy(d_ifft_input_1, ifft_input_1, sizeof(complex)*window_size, cudaMemcpyHostToDevice);
			cudaMemcpy(d_ifft_input_2, ifft_input_2, sizeof(complex)*window_size, cudaMemcpyHostToDevice);
			cudaMemcpy(d_scale, scale, sizeof(complex)*window_size, cudaMemcpyHostToDevice);
			fft_stage << < dimGrid, dimBlock >> > (d_ifft_input_1, d_ifft_input_2, d_scale, d_ifft_output, window_size);
			cudaMemcpy(ifft_output, d_ifft_output, sizeof(complex)*window_size, cudaMemcpyDeviceToHost);
		}
		fft_reorder(ifft_output, restore_signal + s * window_size, window_size, stage, true);
		 
		for (i = 0; i < window_size; i++) {
			fprintf(time_list, "%lf %lf j\n", (restore_signal + s * window_size + i)->real, (restore_signal + s * window_size + i)->imaginary);
		}
	}
	cudaFree(d_ifft_table); cudaFree(d_ifft_input_1); cudaFree(d_ifft_input_2); cudaFree(d_scale); cudaFree(d_ifft_output);
	free(ifft_table); free(ifft_input_1); free(ifft_input_2); free(scale); free(ifft_output); free(freq_signal);

	//Restore
	j = 0;
	short *answer = (short *)calloc(*amount, sizeof(short));
	for (i = 0; i < second*window_size; i++) {
		if ((i % window_size) < *Sample_rate) {
			*(answer + j) = (short)((restore_signal + i)->real);
			j++;
		}
		if (j == *amount) break;
	}
	write_wav_data(output, answer, *amount);
	free(answer); free(restore_signal);
	
	fclose(input); fclose(output); fclose(frequency_list); fclose(time_list);
