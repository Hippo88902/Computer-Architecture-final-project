...
void paralled_idct_stage(complex *freq_signal, complex *idft_output, complex *idft_table, int window_size, int count) {
	for (int i = 0; i < window_size; i++) {
		*(idft_output + i) = complex_add(*(idft_output + i), complex_mul(*(idft_table + ((count * i) % window_size)), *(freq_signal + count)));
	}
}
...
int main()
{
	...
	for (s = 0; s < second; s++) {
		for (i = 0; i < window_size; i++) {
			paralled_idct_stage(freq_signal + window_size * s, idft_output + window_size * s, idft_table, window_size, i);
